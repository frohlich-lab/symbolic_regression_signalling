"""PySR hyperparameter grid search on high-performing functional markers.

This script selects markers whose PySR models achieved log-space R² > 0.7 for
both "gfp" and "all" feature modes, samples three markers at random, and runs a
...
- Strip trailing `_fit` from dataset columns to match functional-markers script.
...
from pipelines.experimental.sr_pipeline.run_functional_markers import (
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--marker-definitions-csv", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)

    # Seed handling: keep --seed for compatibility, add --random-state as alias.
    # If both omitted → default 42 (matches functional-markers script).
...
def _prepare_marker_dataset(
    ...
    if subset.empty:
        raise ValueError("Selected marker has no data after filtering.")
...
def _run_pysr_model(
    ...
    # Start from global config; override with sweep params and alignments to the functional_markers script
    ...
    config.update(
        {
            ...
            "binary_operators": ["+", "-", "*", "/"],
            "unary_operators": [],           # functional-markers default: none
            "maxsize": 20,
            "parsimony": 0.8,
            "batching": True,
            "annealing": True,
...
def _pick_candidate_markers(summary: pd.DataFrame, seed: int) -> List[str]:
    pysr_df = summary[summary["model"].str.lower() == "pysr"].copy()
    markered = pysr_df.markerby(["marker_name", "feature_mode"])["test_log_r2"].mean().unstack()
    qualified = markered[(markered.get("gfp", 0) >= 0.7) & (markered.get("all", 0) >= 0.7)].index.tolist()
    if len(qualified) < 3:
        raise ValueError("Fewer than three markers satisfy the R² threshold in both modes.")
...
def _plot_heatmaps(
    ...
    if not results_by_mode:
        LOGGER.warning("No heatmap data available for marker '%s'; skipping figure.", marker_name)
        return
    ...
    if not ordered_modes:
        LOGGER.warning("No feature modes produced heatmaps for marker '%s'; skipping.", marker_name)
        return

    ncols = max((len(results_by_mode[mode]) for mode in ordered_modes), default=0)
    if ncols == 0:
        LOGGER.warning("Heatmap grids empty for marker '%s'; nothing to plot.", marker_name)
        return
    ...
    fig.suptitle(f"{marker_name} • PySR hyperparameter grid", fontsize=14)
    fig.tight_layout(rect=[0, 0, 1, 0.95])

    slug = "".join(c if c.isalnum() or c in ("-", "_") else "_" for c in marker_name)
...
def main() -> None:
    ...
    selected_markers = _pick_candidate_markers(summary, rs)
    LOGGER.info("Selected markers: %s", ", ".join(selected_markers))

    LOGGER.info("Loading dataset %s", args.dataset)
    data = pd.read_csv(args.dataset)

    # *** CHANGE 1: Strip trailing `_fit` to match functional-markers preprocessing ***
    ...
    # Drop rows with missing targets (functional-markers behavior)
    before_rows = len(data)
    data = data.dropna(subset=["p-ERK1-2_dt"])
    LOGGER.info("Retained %d/%d rows after target drop", len(data), before_rows)

    # Zero tiny targets before balancing (same as functional-markers)
    ...
    LOGGER.info("Loading marker definitions from %s", args.marker_definitions_csv)
    marker_markers = load_marker_markers(
        balanced,
        json_path=None,
        csv_path=args.marker_definitions_csv,
    )
    ...
    for marker_name in selected_markers:
        markers = marker_markers.get(marker_name)
        if not markers:
            LOGGER.warning("Group '%s' missing in definitions; skipping.", marker_name)
            continue

        marker_slug = "".join(c if c.isalnum() or c in ("-", "_") else "_" for c in marker_name)
        marker_dir = output_dir / marker_slug
        marker_dir.mkdir(parents=True, exist_ok=True)

        LOGGER.info("Processing marker '%s' (%d markers)", marker_name, len(markers))
        marker_heatmap_results: Dict[str, Dict[int, np.ndarray]] = {}

        for feature_mode in args.feature_modes:
            feature_cols = select_features(balanced, feature_mode, gfp_columns)
            if not feature_cols:
                LOGGER.warning("No features for mode '%s'; skipping marker '%s'", feature_mode, marker_name)
                continue

            try:
                X_train, X_test, y_train, y_test, _ = _prepare_marker_dataset(
                    balanced, markers, feature_cols, args.test_size, rs
                )
            except ValueError as exc:
                LOGGER.warning("Skipping marker '%s' mode '%s': %s", marker_name, feature_mode, exc)
                continue
            ...
            for niter in niterations_grid:
                ...
                summary_table = Table(title=f"Results Summary • {marker_name} • {feature_mode}")
                ...
                for rec in [r for r in results_records if r["marker"] == marker_name and r["feature_mode"] == feature_mode]:
                    val = rec["test_log_r2"]
            ...
            if heatmap_data:
                marker_heatmap_results[feature_mode] = heatmap_data

        if marker_heatmap_results:
            heatmap_dir = marker_dir / "heatmaps"
            _plot_heatmaps(
                marker_heatmap_results,
                populations_grid,
            ...
            LOGGER.warning("No heatmaps generated for marker '%s'; skipping combined figure.", marker_name)
