...
def _plot_noise_lineplot_for_metric(
    ...
    def _render_lineplot(*, fixed_ylim: bool) -> None:
        ...
        ax.set_xlabel("Noise Regime", fontsize=label_fs)
        ...
        plt.savefig(plot_dir / f"{spec.filename_prefix}_noise_regime_lineplot{fixed_suffix}{suffix}.png", dpi=300)
        plt.close()
...
def _plot_error_distributions(metric_key, metric_distributions, output_dir):
    ...
    plt.suptitle(f"{spec.label} Error Distributions by Noise Regime", fontsize=32)
...
def plot_model_subregimes(model_dict, output_dir):
    ...
    fig.suptitle("Model Error Landscape by Noise Regime and Model", fontsize=32, y=1.02)
    plt.tight_layout()
...
def plot_input_error_correlation(model_dict, output_dir):
    ...
    plt.suptitle("Feature–Error Correlations by Noise Regime", fontsize=32)
...
def plot_model_error_correlation(model_dict, output_dir):
    ...
    plt.suptitle("Model–Model Error Correlations by Noise Regime", fontsize=32)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
...
def evaluate_models(
    ...
    *,
    ...
    message = f"Noise regimes completed across {len(seeds)} seed runs. Outputs written to {output_dir}"
    if not render_plots:
