# mm_deviation_regimes.py
"""
Symbolic regression across biochemical *deviation* regimes (MM error bins),
...
Usage:
    python mm_deviation_regimes.py \
...
LOGGER = regime_logger("pysr_regimes")
...
def _mm_log_error(df: pd.DataFrame):
    ...
    mm_pred = sqssa_only(df)
    return np.abs(np.log(np.maximum(mm_pred, EPS)) - np.log(np.maximum(target, EPS)))

def _filter_by_mm_error(df: pd.DataFrame, low: Optional[float] = None, high: Optional[float] = None):
...
def plot_deviation_lineplots(distributions, seed_medians, seed_means, output_dir, template=False):
    for mkey in metric_keys():
...
def plot_input_error_correlation(model_dict, output_dir):
    ...
    if all_error_df:
        ...
        plt.title("Overall Feature–Error Correlation (All Deviation Regimes)")
        plt.tight_layout()
...
def plot_model_error_correlation(model_dict, output_dir):
    ...
    if all_model_error_df:
        ...
        sns.heatmap(overall_corr, annot=True, cmap="coolwarm", center=0)
        plt.title("Overall Model–Model Error Correlation (All Deviation Regimes)")
        plt.tight_layout()
...
def evaluate_models(
    ...
    *,
    ...
    if render_plots:
        ...
        plot_deviation_lineplots(distributions, seed_medians, seed_means, output_dir, template=False)
        plot_deviation_lineplots(distributions, seed_medians, seed_means, output_dir, template=True)
        if len(VARIANTS) > 1:
...
def evaluate_models_multi(
    ...
    *,
    ...
    if render_plots:
        ...
        distributions, seed_medians, seed_means = _collect_error_distributions(base_model_dict)
...
# ── CLI ────────────────────────────────────────────────────────────────────────
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate deviation regimes with PySR/MM/NN (sQSSA/tQSSA-aware), unified logging & plots.")
    ...
    output_dir = Path(args.output_dir) if args.output_dir else base_dir / "mm_deviation_regimes"
