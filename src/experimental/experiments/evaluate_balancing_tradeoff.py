...
``run_functional_groups.py`` while allowing the maximum samples per bin to vary.
...
Example usage:
    python src/experimental/evaluate_balancing_tradeoff.py \\
...
def _get_sanitized_gfp_columns(args: argparse.Namespace) -> List[str]:
    ...
    # fall back to default used in run_functional_groups.py when nothing provided
...
def filter_group_rows(rows: Sequence[DataRow], group_markers: Sequence[str]) -> List[DataRow]:
    marker_set = set(group_markers)
    return [row for row in rows if row.marker in marker_set]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--run-pysr",
        action="store_true",
        help="For each configuration, run run_functional_groups.py and collect PySR metrics.",
    )
    parser.add_argument(
        "--pysr-script",
        type=Path,
        default=Path("src/experimental/sr_pipeline/run_functional_groups.py"),
        help="Path to run_functional_groups.py when --run-pysr is enabled.",
    )
    parser.add_argument(
        "--pysr-output-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=["gfp"],
        help="Feature modes to pass to run_functional_groups.py.",
    )
    parser.add_argument(
        "--pysr-gfp-columns",
        nargs="*",
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=["pysr"],
        help="Models to request when running run_functional_groups.py.",
    )
    parser.add_argument(
        "--pysr-random-state",
        type=int,
        default=42,
        help="Random seed passed to run_functional_groups.py.",
    )
    parser.add_argument(
        "--pysr-test-size",
        type=float,
        default=0.2,
        help="Test-size fraction passed to run_functional_groups.py.",
    )
    parser.add_argument(
        "--min-bin-samples",
        type=int,
        default=500,
        help="Minimum samples per OOM bin (also forwarded to run_functional_groups.py).",
    )
    parser.add_argument(
        "--uncapped-max-value",
        type=int,
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="Additional arguments appended when invoking run_functional_groups.py.",
    )
    parser.add_argument(
        "--skip-existing",
        action="store_true",
...
def run_pysr_for_cap(
    ...
    """Execute run_functional_groups.py with a specific max-bin cap and parse metrics."""
    output_dir = args.pysr_output_root / f"cap_{cap_label}"
    summary_path = output_dir / "reports" / "summary" / "functional_group_summary.csv"
